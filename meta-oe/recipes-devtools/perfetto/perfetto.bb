...
CFLAGS:append:arm = "${@bb.utils.contains('TUNE_FEATURES', 'callconvention-hard', ' -mfloat-abi=hard', '', d)}"
LDFLAGS:append:arm = "${@bb.utils.contains('TUNE_FEATURES', 'callconvention-hard', ' -mfloat-abi=hard', '', d)}"
...
do_configure () {
    ...
    ARGS=$ARGS" target_cc=\"$CC_BIN ${CFLAGS}\""
    ARGS=$ARGS" target_cxx=\"$CXX_BIN -std=c++11 ${CXXFLAGS}\""
    ARGS=$ARGS" target_strip=\"$STRIP_BIN\"" #
    ARGS=$ARGS" target_sysroot=\"${RECIPE_SYSROOT}\""
    ARGS=$ARGS" target_linker=\"$CC_BIN ${LDFLAGS}\""
    ...
    if [ "${TOOLCHAIN}" = "clang" ]; then
        BB_CLANGXX="${BUILD_CXX} ${BUILD_LDFLAGS} ${CXXFLAGS}"
        BB_CLANG="${BUILD_CC} ${CFLAGS}"
        BB_LLVM_OBJCOPY="${RECIPE_SYSROOT_NATIVE}/usr/bin/llvm-objcopy"
        
        HOST_CLANGXX="${STAGING_DIR_NATIVE}/usr/bin/clang++ -stdlib=libc++ -rtlib=libgcc -unwindlib=libgcc ${CXXFLAGS}"
        HOST_CLANG="${STAGING_DIR_NATIVE}/usr/bin/clang ${CFLAGS}"
    ...
    # Quick hack for musl, due to musl not supporting strtoll_l and strtoull_l
    if [ "${TCLIBC}" = "musl" ]; then
